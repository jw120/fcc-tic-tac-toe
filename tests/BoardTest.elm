...
allTests =
  ...
  in
    suite
      ...
      , suite "countPieces"
        [ test "X counts" <| assertEqual [0, 1, 0, 1, 2, 3] <| List.map (Board.pieceCount X) [b0, x1, o1, b2, b3, w3]
        , test "O counts" <| assertEqual [0, 0, 1, 1, 1, 0] <| List.map (Board.pieceCount O) [b0, x1, o1, b2, b3, w3]
        ]
     , suite "isFull"
